<x-guest-layout>
    <div class="flex pt-5 mobile:flex mobile:flex-col mobile:p-5 mobile:justify-center mobile:w-full mobile:text-center tablet:flex tablet:flex-col">
        <div class="pl-[100px] pb-[50px] mobile:p-0 mobile:flex mobile:flex-col mobile:items-center mobile:pb-5 tablet:flex tablet:items-center tablet:flex-col">
            <h1 class="bg-[#2b364d] max-w-[400px] p-5 font-bold text-3xl rounded-t-3xl mobile:w-full tablet:w-full">Wie ben ik</h1>
            <p class="text-white max-w-[400px] bg-[#2b364d] p-5 rounded-b-3xl ">
                Lorem ipsum dolor sit amet. Qui nihil earum vel rerum dolore et voluptas autem aut odit quae At cumque
                quam
                ut sint libero. Ad placeat dolores aut laudantium repellat est sequi alias et minima blanditiis nam
                voluptatem laboriosam et impedit nemo qui ipsa dolores? Rem itaque laborum est consectetur perferendis
                ex
                inventore recusandae. At placeat veniam aut soluta sapiente et odio consequuntur.
                Et nostrum culpa aut corporis voluptate non impedit provident. Et nulla illum aut aliquam dolores 33
                culpa
                ipsa. Ab consequatur tempore ea eaque molestias sit error doloremque et optio dolor est minima eligendi
                et
                iste cupiditate sit aliquam veniam!
                Ea perferendis odit et reprehenderit dicta qui illum deleniti et nesciunt nisi non quia reprehenderit ut
                molestias necessitatibus. In dolorem vero est magni similique et laboriosam dolorum! Sed quod molestiae
                eum
                dolorum Quis et natus assumenda 33 fuga harum et neque sint?
            </p>
        </div>
        <div class="w-[500px] flex justify-center items-center mobile:hidden tablet:hidden">
            <img class="rounded-full" src="data:image/jpeg;base64," alt="foto niet geladen">
        </div>
        <div class="flex ml-auto mr-[100px] bg-[#2b364d] mb-[50px] rounded-3xl p-5 max-w-[500px] mobile:items-center mobile:flex mobile:w-full mobile:justify-center tablet:flex tablet:items-center tablet:flex-col tablet:w-full">
            <form class="flex flex-wrap" action="{{ route('contact.create') }}" method="post">
                @csrf
                <div class="text-5xl font-bold">
                <p>Stuur mij een bericht</p>
                </div>
                <div class="w-1/5">
                    Name:
                </div>
                <div class="w-4/5">
                    <input class="w-full rounded-xl text-black mobile:w-40" type="text" name="name" value="{{ old('name') }}">
                </div>
                @error('name')
                {{$message}}
                @enderror
                <div class="w-1/5">
                    E-mail:
                </div>
                <div class="w-4/5">
                    <input class="w-full rounded-xl text-black mobile:w-40" type="text" name="email" value="{{ old('email') }}">
                </div>
                @error('email')
                {{$message}}
                @enderror
                <div class="w-1/5">
                    Message:
                </div>
                <div class="w-4/5">
                    <textarea class="w-full rounded-xl h-24 text-black mobile:w-40" name="message">{{ old('message') }}</textarea>
                </div>
                @error('message')
                {{$message}}
                @enderror
                <br>
                <div class="pl-[225px] mobile:flex mobile:justify-center mobile:w-full mobile:pl-0">
                <button class="bg-[#111827] hover:bg-[#0c111b] text-white font-bold py-2 px-4 border-b-4 rounded" type="submit">send</button>
                </div>
            </form>
        </div>
        @if (@session('message'))
            <h1>{{ @session('message') }}</h1>
        @endif
    </div>

</x-guest-layout>
